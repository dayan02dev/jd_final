...
from config.settings import LINKEDIN_CLIENT_ID, LINKEDIN_CLIENT_SECRET
from config.settings import LINKEDIN_ACCESS_TOKEN, LINKEDIN_COMPANY_ID
...
class JobPoster:
    ...
    def setup_platforms(self):
        ...
        if self.platforms["linkedin"]["enabled"]:
            try:
                ...
                # Test the API connection with a simple request (e.g., get company details)
                test_url = f"https://api.linkedin.com/v2/organizations/{LINKEDIN_COMPANY_ID}"
                response = requests.get(test_url, headers=self.linkedin_headers)
                
                if response.status_code == 200:
                    logger.info("LinkedIn API client set up successfully")
                else:
                    logger.error(f"LinkedIn API connection test failed: {response.status_code} - {response.text}")
                    self.platforms["linkedin"]["enabled"] = False
            except Exception as e:
                logger.error(f"Failed to set up LinkedIn API client: {str(e)}")
                self.platforms["linkedin"]["enabled"] = False
    ...
    def format_for_platform(self, job_description: str, platform: str) -> Dict:
        ...
        if platform == "twitter":
            ...
            # Extract necessary information from the job description
            lines = job_description.split("\n")
            ...
            for line in lines:
                if "# **" in line or "**Position:**" in line:
                    role_title = line.replace("# **", "").replace("**", "").replace("Position:", "").strip()
                elif "**Location:**" in line:
                    location = line.replace("**Location:**", "").strip()
                elif "**Employment Type:**" in line:
                    employment_type = line.replace("**Employment Type:**", "").strip()
            
            # Get description from the job_description
            # For LinkedIn, we need to convert markdown to a simplified HTML format
            description = job_description
            
            # Map employment type to LinkedIn's format
            linkedin_employment_type = "FULL_TIME"
            if "Part-Time" in employment_type:
                linkedin_employment_type = "PART_TIME"
            elif "Contract" in employment_type:
                linkedin_employment_type = "CONTRACT"
            elif "Freelance" in employment_type:
                linkedin_employment_type = "CONTRACT"
            
            # Format the job for LinkedIn's API
            return {
                ...
                "locationPlaces": [
                    {
                        "country": "US",  # Default to US, would need to be parsed from location
                        "localizedName": location
                    }
                ],
                "employmentType": linkedin_employment_type,
                "workplaceType": "REMOTE" if "Remote" in location else "ONSITE"
            }
        ...
        for platform, config in self.platforms.items():
            ...
            try:
                ...
                if platform == "twitter":
                    ...
                    post_url = "https://api.linkedin.com/v2/jobPostings"
                    response = requests.post(post_url, headers=self.linkedin_headers, json=formatted_job)
                    
                    if response.status_code in [200, 201]:
                        job_id = response.json().get("id", "Unknown")
                        success = True
                        message = f"Posted to LinkedIn. Job ID: {job_id}"
                    else:
                        success = False
                        message = f"Failed to post to LinkedIn. Status: {response.status_code}, Response: {response.text}"
    ...
    def mock_post_job(self, job_description: str) -> Dict[str, Dict]:
        ...
        for platform in self.platforms.keys():
            ...
            if platform == "twitter":
                ...
                preview = f"Would post job: \"{formatted_job['title']}\" to LinkedIn company page {LINKEDIN_COMPANY_ID}"
            elif platform == "google_jobs":
